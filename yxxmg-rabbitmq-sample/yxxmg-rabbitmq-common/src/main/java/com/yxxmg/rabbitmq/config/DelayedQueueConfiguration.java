...
public class DelayedQueueConfiguration {
    private static final String DELAYED_QUEUE_BEAN_NAME_SUFFIX = ".delayed.queue";
    private static final String DELAYED_QUEUE_EXCHANGE_NAME_SUFFIX = ".delayed.exchange";

    private static final String DELAYED_BINDING_BEAN_NAME_SUFFIX = ".delayed.binding.queue";

    @Resource
    private GenericApplicationContext genericApplicationContext;

    @Bean
    public List<DelayedQueue> delayedQueueList() {
        return Stream.of(QueueEnum.values()).map(this::createDelayedQueue).collect(Collectors.toList());
    }

    private DelayedQueue createDelayedQueue(QueueEnum queueEnum) {
        // 创建优先级队列
        ...
        return new DelayedQueueWrapper(getExchangeName(queueEnum), queueEnum);
    }
    ...
    private static String getBindingBeanName(QueueEnum queue) {
        return queue.name() + DELAYED_BINDING_BEAN_NAME_SUFFIX;
    }

    private String getExchangeName(QueueEnum queue) {
        return queue.name() + DELAYED_QUEUE_EXCHANGE_NAME_SUFFIX;
    }

    private void createExchange(QueueEnum queue) {
        DirectExchange directExchange =
            new DirectExchange(queue.name() + DELAYED_QUEUE_EXCHANGE_NAME_SUFFIX, true, false, Maps.newHashMap());
    ...
    private void createQueue(QueueEnum queueEnum) {
        ...
        Queue queue = new Queue(DelayedQueue.queueName(queueEnum), true, false, false, args);
    ...
    private static String getQueueBeanName(QueueEnum queueEnum) {
        return queueEnum.name() + DELAYED_QUEUE_BEAN_NAME_SUFFIX;
    }

    @AllArgsConstructor
    static class DelayedQueueWrapper implements DelayedQueue {
